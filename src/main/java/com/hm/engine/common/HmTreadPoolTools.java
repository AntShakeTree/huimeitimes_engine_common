 ...
 *	TODO:未完待续
 */
public final class HmTreadPoolTools {
	//先写死
	...
	private static ExecutorService MONITOR_TREAPOOL=Executors.newFixedThreadPool(10);

	public static <V> Future<V> monitorSubmit(Callable<V> task) {
		return MONITOR_TREAPOOL.submit(task);
	}

	//小于10秒的统一算是短消息。
	public static  void monitorExecute(Runnable command){
		MONITOR_TREAPOOL.execute(command);
	}
